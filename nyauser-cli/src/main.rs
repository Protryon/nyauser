...
use nyauser_types::{Profile, RegexWrapper, Series, SeriesStatus, PullEntryNamed, PullState, Episode, PullEntryFilter};
use regex::Regex;
use reqwest::{Url, Client, RequestBuilder, Method, StatusCode};
...
async fn main() {
    ...
    env_logger::Builder::new().parse_env(
...
fn api(method: Method, path: impl AsRef<str>) -> Result<RequestBuilder> {
...
impl From<Series> for SeriesTable {
    fn from(value: Series) -> Self {
        Self {
            ...
            max_days_old: value.max_days_old.map(|x| x.to_string()).unwrap_or_default(),
            relocate: value.relocate.unwrap_or_default(),
...
impl From<PullEntryNamed> for PullTable {
    fn from(value: PullEntryNamed) -> Self {
        Self {
            ...
            torrent_id: value.pull_entry.torrent_id.map(|x| x.to_string()).unwrap_or_default(),
...
async fn execute_mode(mode: &Mode) -> Result<()> {
    ...
    match mode {
        Mode::Profile { mode } => match mode {
            ProfileMode::List => {
                let response = api(Method::GET, "/api/v1/profile")?
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
            ...
            ProfileMode::Set { name } => {
                let response = api(Method::GET, format!("/api/v1/profile/{}", urlencoding::encode(name)))?.send().await?;
                let current: Profile = if response.status().is_success() {
                    ...
                    bail!("Got HTTP Status on get: {}\n{}", response.status(), response.text().await?);
                };
                let output = scrawl::with(&serde_yaml::to_string(&current)?).map_err(|e| anyhow!("{e}"))?;
                let output = output.to_string().map_err(|e| anyhow!("{e}"))?;
                let mut new: Profile = serde_yaml::from_str(&output)?;
                new.name = name.clone();
                let response = api(Method::POST, format!("/api/v1/profile/{}", urlencoding::encode(name)))?
                    .json(&new).send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status on update: {}\n{}", response.status(), response.text().await?);
                }
            },
            ProfileMode::Delete { name } => {
                let response = api(Method::DELETE, format!("/api/v1/profile/{}", urlencoding::encode(name)))?
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
        ...
        Mode::Pull { mode } => match mode {
            PullMode::List { profile, title_contains, title_is, season_is, episode_is, state } => {
                let response = api(Method::GET, "/api/v1/pull")?
                    .query(&PullEntryFilter {
                        ...
                        state: state.as_ref().map(|x| x.parse()).transpose().map_err(|_| anyhow!("invalid state"))?,
                    })
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
            ...
            PullMode::Delete { id } => {
                let response = api(Method::DELETE, format!("/api/v1/pull/{}", urlencoding::encode(id)))?
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
        ...
        Mode::Series { mode } => match mode {
            SeriesMode::List => {
                let response = api(Method::GET, "/api/v1/series")?
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
            ...
            SeriesMode::Get { name } => {
                let response = api(Method::GET, format!("/api/v1/series/{}/status", urlencoding::encode(name)))?.send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
            ...
            SeriesMode::Set { name } => {
                let response = api(Method::GET, format!("/api/v1/series/{}", urlencoding::encode(name)))?.send().await?;
                ...
                new.name = name.clone();
                let response = api(Method::POST, format!("/api/v1/series/{}", urlencoding::encode(name)))?
                    .json(&new).send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status on update: {}\n{}", response.status(), response.text().await?);
                }
            ...
            SeriesMode::Delete { name } => {
                let response = api(Method::DELETE, format!("/api/v1/series/{}", urlencoding::encode(name)))?
                    .send().await?;
                if !response.status().is_success() {
                    bail!("Got HTTP Status: {}\n{}", response.status(), response.text().await?);
                }
